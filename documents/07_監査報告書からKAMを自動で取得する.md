...
会計士などを除く、多くの人はあまり監査報告書やKAMというものに関わりがなくわからない方も多いと思います。

そのため、この2つがどういったものなのか押さえておきましょう。
...
簡単にいえば、**「この有価証券報告書はお金のプロである公認会計士から認められた正しく作られている信用できるものですよ！」**と証明するためのものです。
そのため、書類の中身には担当した会計士の名前や監査意見、その根拠などが書いてあります。
...
投資家などからすれば有価証券報告書の本文と比べると参照されるケースは少ないですが、財務諸表の監査において特に重要なことがKAMに書かれているため、監査報告書の中では重要視されることが多いです。

企業ごと、事業ごとで行っている業務の体系や詳細は変わってくるでしょう。それらをどうルールに当てはめるのか、その時に懸念しなくてはならなかった点などが書かれています。以下の画像のすべてがKAMに当たります。その中でも表の部分が注目され、中身だけでなくカラムも企業や年度によっても異なってくるため注目されることがあります。
そのため、この見出しだけでも分析ができるほどなのです。
...
監査報告書とKAMについてどのようなものなのか分かったところで実際にデータを取得できるようになりましょう。

## タクソノミを確認しよう

本章でも、取得したい情報を示すタクソノミを指定しましょう。

今回は2章で紹介したタクソノミの参照方法の3つ目を使用してタクソノミを参照します。
...
次にXBRLファイルを選ぶ時なのですが、監査報告書には連結と個別の区別はタクソノミ単位ではなく、ファイル単位で分かれています。つまり、今までの正規表現のコードを使用し、PublicDocをAuditDocに変えただけでは正しくファイル指定できないということです。
...
EDIENTコードと企業名は有報から取得していますが、KAMは監査報告書から取得するためfor文も別で書く必要があります。

またif文の中で先ほど調べたタクソノミを指定します。また、KAMはテキストデータであるためデータを綺麗に整える処理も忘れずに行いましょう。

データをリストに入れる際は最後に入れましょう。最後に入れることで同じリストに入れることができます。逆言えば、EDIENTコードと企業名を見つけた段階でリストに入れてもらうと別のリストで保管してしまいます。
