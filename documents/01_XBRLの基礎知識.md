...
最近ではこのテキストデータを用いた分析等のニーズが増え、注目がされています。

例えば

    ”「事業等のリスク」や「ESG項目」などのテキストデータの変化に応じて「業績」や「株価」の数値にも影響を及ぼすのではないか”

というような分析が多く行われています。
実際に公開されている研究だと”渡部 美紀子「Covid-19パンデミックに関する有価証券報告書「事業等のリスク」の記載の変化」”などの研究があり、コロナウイルスが蔓延していた時期に「事業等のリスク」にそれに関係することを「記述した企業」と「していない企業」でどのような違い変化があるのかが研究されています。
...
しかし、現在ではXBRLの登場によりこれらのデメリットは解消され、むしろ利用に際してメリットが生まれました。
