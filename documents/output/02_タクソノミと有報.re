...
例えば、2023年度まではESG @<fn>{fn1}に関する標準タクソノミが準備されておらず、提出者別タクソノミを設定していました。そういった状況の中ではタクソノミが設定されず抜け落ちてしまうケースも実際にありました。また、勘定科目においても同様で、タクソノミに記載されていないような特殊な勘定科目にはタグがついていないことがあります。

==== 抜き出す内容がわかってないといけない

今度は全く別の角度からの欠点です。情報の取捨選択ができる知見を持たないと上記2つの欠点にも気づくことすらできないという点です。いくらタクソノミがきれいに指定してあっても情報を取ってくる人が取って来る情報そのものについて知らなければ意味がありません。取って来る情報の対象である有価証券報告書等に関するドメイン知識は必須と言えるでしょう。また、有報だけ知っていてもタクソノミとインスタンスという概念やタクソノミの参照方法も知らずには情報は取れません。公認会計士などは有価証券報告書を読むことに慣れているかもしれませんが、タクソノミとの紐づけが出来ていない人が多いと推測されます。タクソノミのことを勘定科目マスタと考えている人も実は多いです。どちらの知識も正確に身に着けることが大事なのです。

そういったわけで、次の章からは調べる対象を有報に絞って、有報についてとタクソノミの参照方法について説明しようと思います。

== 有価証券報告書とは

有価証券報告書とは、事業年度ごとに企業自ら財務情報や経営状況を外部に開示するための資料です。主に事業年度が切り替わってから前年度分の総まとめとして提出されるものです。基本的に事業内容が変われば、有報の書かれる内容も変わるため、構造自体が変わるということもあります。有報を作成する際に目次は決まっていますが、その中の項目は各企業で変わってくるためこの中から知りたい情報を探すということになってきます。有報を作成する多くは、印刷会社（宝印刷、プロネクサス）のシステムを用いたうえで、ASBJ@<fn>{fn2} や印刷会社が出している記載要領を等を参照して作ります。そのため、原則的に記載内容が似ているケースが多いですが、細かいところの書きぶりは変わってきます。
...
//image[0203_EDINET画面][EDINETのトップページ画面]{
//}

=== よく見られる項目

財務情報であれば、「経理の状況」に財務諸表等が記載されています。有報の特徴として、テキストデータが豊富ということが挙げられます。テキストデータとはいわゆる文章での情報です。テキストデータに注目が集まる理由としては、数値だけでは説明しきれない状況や背景などについて説明できるという点が挙げられます。人はこの文章の情報を読み取ることができますが、機械にとってはこの文章の意味はわかりませんでした。しかし、自然言語処理技術の発達により、このテキストデータを用いた分析を機械ができるようになってきており、テキストデータにより注目が集まっています。
...
例えば、特別損失の勘定科目などはタクソノミで取れないケースが多いでしょう。なぜならば特別損失に記載されている勘定科目は、企業特有のものが設定されている場合があるためです。例として「プロ野球選手移籍金」などは標準タクソノミに存在しないため、提出者別タクソノミによって追加され掲載されたことがあります。提出者別タクソノミでは企業ごとに独自のタクソノミが追加されているため、基本的に同じ勘定科目だったとしても同じタクソノミが設定されていません。
...
//image[0205_CSVファイルのダウンロード][CSVファイルのダウンロード]{
//}

その中から圧縮サイズが一番大きいファイルを開き以下のような表を参照できる状態にしてください。

//image[0206_タクソノミCSV][タクソノミCSV]{
//}
...
@<embed>{|html|<img width=450 alt="PublicDocを選択" src="images/0207_PublicDoc.png">}

//endchild

 4. XBRLファイルを探す（種類のカラムを参照するとわかりやすいです）※ややこしいので注意

//beginchild

@<embed>{|html|<img width=450 alt="XBRLファイルを探す" src="images/0208_XBRLファイルを探す.png">}
...
※Wordはダメ（検索できなかったり、どれがタクソノミかわからなかったりする）　メモや検索エンジンではダメではないが色がついていない分見づらく感じた。
...
//image[0209_XBRLファイル内の項目を探す][XBRLファイル内から項目を検索する]{
//}

例えば「事業等のリスク」などで検索し、タクソノミを探します。

//image[0210_事業等のリスクを探す][「事業等のリスク」の項目を探す]{
//}
...
contextRef="FilingDateInstant”@<fn>{fn3}
...
@<href>{https://prod-files-secure.s3.us-west-2.amazonaws.com/8462a5f5-2269-4832-abed-8bac1cfbc489/08e36eb0-db99-455a-974f-794aa1a2c792/1e_ElementList.xlsx,1e_ElementList.xlsx}

有価証券報告書以外の書類についての情報も載っているため、まずは有報の情報にたどり着くための手順について説明します。

ファイルをダウンロードし開くと、以下のような目次のシートが表示されます。

//image[images\0211_タクソノミ要素リスト目次][タクソノミ要素リスト]{
//}

目次から「9 企業内容等の開示に関する内閣府令 第三号様式 有価証券報告書（jpcrp03000-asr)」のシートに移ります。「詳細ツリー‐標準ラベル（日本語）」（B列）を対象に知りたい項目を検索します。
...
なぜなら、前者では、財務・非財務どちらの情報に対しても取得はできますが、CSVの内容が必ずしも合っている保証がなく、まれに間違っている場合もあるという大きな欠陥があるため、後者では、財務情報を取得したい場合に勘定科目を検索してもインスタンスと別の場所に格納されてていることから勘定科目から逆算的にタグを知ることができないためです。逆に、どれがタグで、どれがcontext_refで、このタグの意味はこうと理解出来るようになったら、XBRLフォルダの中のPublicDocを開く方法の情報量にも惑わされないと思います。このように一長一短ではあるので場合に分けてデータを取得する方法を選ぶのを推奨です。
...
//footnote[fn1][ESGとは、Environment(環境)、Social(社会)、Governance(ガバナンス（企業統治）)を考慮した投資活動や経営・事業活動のこと。引用：内閣府ホームページ2.2_ESGの概要]
//footnote[fn2][正式名称は企業会計基準委員会。日本の会計基準などを作る機関。]
//footnote[fn3][「contextRef="FilingDateInstant”」は当期の意味。]
