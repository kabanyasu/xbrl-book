...
//image[0701_監査報告書の例][監査報告書の例（2023年度トヨタ自動車株式会社）]{
//}
...
//image[0702_KAMの例_1][KAmの例1][scale=0.8]{
//}

//image[0703_KAMの例2][KAmの例2][scale=0.8]{
//}
...
//image[0704_タクソノミ要素リストの目次][タクソノミ要素リスト目次]{
//}

（2）「Ctrl」＋「F」で検索窓から「監査上の主要な検討事項」と検索（3）「詳細ツリー‐標準ラベル（日本語）」（B列）でヒットするものを探す

//image[0705_タクソノミ要素リスト内で目的の項目を検索][タクソノミ要素リスト内で検索]{
//}

（4）「冗長ラベル（日本語）」（C列）で連結のものと個別のものが分かれていることが確認できるため連結のものの「要素名」（I列）を確認

//image[0706_タクソノミ要素リストの中から参照する][タクソノミ要素リストから参照]{
//}
...
//image[0707_タクソノミ連結個別の指定][タクソノミで連結・個別の指定をする]{
//}
