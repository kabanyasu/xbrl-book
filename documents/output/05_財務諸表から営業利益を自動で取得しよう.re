...
財務データを取得する際には、以下を気をつける必要があります。

 * 適用される会計基準は何なのか
 * 連結財務諸表からデータを取得するか、単体財務諸表からデータを取得するか
 * 勘定科目は揃っているか

上記の3項目が統一されていないと、同じコードでデータを取得することは困難になります。それらの対応のためにコードの条件分岐を実施したりするなど、XBRLを利用する際の苦労はこちらに集約されてるといっても過言ではありません。

今回はトヨタ自動車株式会社を例に、データの取得を試みようと思います。トヨタ自動車株式会社の分析において、連結財務諸表からデータを抜き出そうと考えた所、適用される会計基準は国際会計基準(IFRS)でした。したがって、国際会計基準(IFRS)を適用している連結財務諸表の営業利益を取得するためのコードを書いてみることを決めました。

一気に何社も取得するとコードのどの部分でデータを取得しているのかが理解がしづらいため、トヨタ自動車1社から営業利益のデータを抜き出し、データ取得の必要最低限について学んでいきましょう。

== タクソノミを確認しよう

データを取得するにあたってタクソノミを指定します。財務データの取得には「タクソノミのタグ」と「コンテキストIDという期間などを表す指標」の2つ指定が必要です。このタクソノミを参照する方法については主に2章に記載しているため参考にしてください。本章ではそこで紹介した1つ目の「EDINETからCSVファイルをダウンロードして検索」を用いてタクソノミを参照します。EDINETから有報をダウンロードする画面で「CSV」をクリックし、その有報のCSVファイルをダウンロードします。その中の圧縮サイズが一番大きいファイルを開くと以下のような表が出て来ます。

//image[images\0206_タクソノミCSV][csvデータ]{
//}

この表は、その企業で利用されているタクソノミをまとめたもので、標準タクソノミならず提出者タクソノミも掲載されています。特に重要な項目は以下のとおりです。
...
コード的には要素IDとコンテキストIDがわかればデータの取得が可能になっているが、それだけを見ても内容が人間には判別しづらいため、他の3つのカラムを参照し正しい要素IDとコンテキストIDを見極めます。

では項目名が営業利益である行を探しましょう。項目名を指定して検索をしないと、別の列にはいっている営業利益という単語も拾ってしまうため、ここは留意が必要です。

//image[images\0501_CSVから営業利益を探す][alt text]{
//}
...
こちらでArelleを利用可能になります。

続いてコードを細かく分けて解説していきます。

まず有報の中から今回取得する項目を抜き出す準備をしています。今回であれば、営業利益の他に「EDINETCODE」「企業名」を取得し、営業利益がどの企業のものなのかわかるようにします。
...
パスとはファイルなどの居場所を示すものであり、VS Codeでそのファイルを右クリックすることで「パスをコピー」と出てくるためそのままコピペしましょう。（相対パスでも可）また、ターミナル上で結果を確認できるようにプリント文を書きます。
...
今回は、トヨタ自動車株式会社に追加して、国際会計基準(IFRS)で連結財務諸表をしている、営業利益が記載されている企業が対象になります。
...
先ほどのコードと異なる点は「ファイルパスを1社から10社に増やすこと」と「全ての企業に対して自動でデータを取得し、次の企業に移り同じ処理を行うこと」の2つです。
...
基本的には先ほどと同じことをしておりますので、ここではコードを変えた箇所について見ていきましょう。
...
正規表現とは、直接パスを通したものだけに処理をするのではなく「パターンに一致するものすべて」に処理をするためのものです。globモジュールはそういった時に使用され、「ｒ」の中のクオーテーションの中がそのパターンの型になっています。また、「＊」はどんな文字が何文字でも入れることができるという意味を持ちます。そのため、ここが固有値に当たる部分を指定するとうまくいきます。
...
この章では、XBRLを使用して有価証券報告書から、企業のEDINETコードと企業名、そして財務情報である営業利益を取得しました。財務データを取得できるようになると、同じ業種などで企業を比較などがしやすくなります。財務データはXBRLファイルを直接見てもタグが見つけられないことが少しネックですが、正しく指定してあげればかなり有効に使えるでしょう。その一方で、会計基準の違いなどによりタグが異なると一気に取得することもできないため、そういった会計のドメイン知識も必要になってきます。また、XBRLのタグは正しく付与されていないことも時々あり、そのせいでデータが上手く取得できないと嘆く人も多いです。多くのXBRLユーザーがXBRLに苦労するところがタグなのです。このタグの操作は実際にやってみないとわからない点が多いので、苦しみながら学習していきましょう。
